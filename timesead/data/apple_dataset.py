...
import os
import hashlib
from typing import Tuple, Optional, Union, Callable, Dict, Any, List, Set
import tempfile
import subprocess
import shutil
import logging
...
class AppleDataset(BaseTSDataset):
    SEQUENCE_WISE_AD = True

    def __init__(self, dataset_path: str = os.path.join(DATA_DIRECTORY, 'AppleDataset'), 
        ...
        self.training_files = sorted(sample(self.training_files, int(len(self.training_files) * 0.8)))
        self.training_split_uid = hashlib.sha256(";".join(self.training_files).encode()).hexdigest()
        self.training_pds = [pd.read_csv(file, skiprows=[1], delimiter=";", index_col='Zeit') for file in sorted(self.training_files)]
        ...
        self.test_files = [
            ...
            for folder in NORMAL_TS_FOLDERS
            for file in sorted(os.listdir(os.path.join(dataset_path, folder)))
            if file.endswith(".csv") and os.path.join(dataset_path, folder, file) not in self.training_files
        ]
        ...
        self.test_targets += [1 for folder in ANOMALOUS_TS_FOLDERS for file in sorted(os.listdir(os.path.join(dataset_path, folder)))]
        self.test_pds = [pd.read_csv(file, skiprows=[1], delimiter=";", index_col='Zeit') for file in self.test_files]
        ...
        #print(f"→ {len(self.training_files)} fichiers en train, IDs = {self.training_files_basenames}")
        #print(f"→ {len(self.test_files)} fichiers en test, IDs = {self.test_files_basenames}")
        #print(f"Répartition normal/anomalie en test : {self.test_targets.count(0)} normals, {self.test_targets.count(1)} anomalies")
    ...
    def load_data(self) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        ...
        for i, data in enumerate(frames):
            if self.training:
                ...
                # target = self.test_targets[i]
                # target = np.asarray(target != 0)
                # target = target.astype(np.int64).repeat(len(data))
                # target = np.asarray((-(i+1) if self.test_targets[i] <= 0 else (i+1))) 
                wltc_index = int(self.test_files_basenames[i].split("_")[1].split(".csv")[0])
                if wltc_index == 0:
