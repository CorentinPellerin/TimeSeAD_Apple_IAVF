...
class DatasetSource(Transform):
    ...
    @property
    def seq_len(self):
        return self.dataset.seq_len[self.start:self.end] if self.axis == 'batch' else [(end - start) for start, end in zip(self.start, self.end)]  # TODO WAS:   self.dataset.seq_len if self.axis == 'batch' else [(end - start) for start, end in zip(self.start, self.end)]
