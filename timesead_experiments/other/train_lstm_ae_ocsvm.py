...
from timesead.models.other import LSTMAEOCSVMAnomalyDetector
from timesead.models.reconstruction import LSTMAEMirza2018
...
def get_anomaly_detector(model, val_loader, training, detector_params, _run, save_detector=True):
    ...
    detector = LSTMAEOCSVMAnomalyDetector(model, **detector_params).to(training.device)
...
def main(model_params, dataset, training, _run, train_detector=True):
    ...
    train_ds, val_ds = load_dataset()
    model = LSTMAEMirza2018(train_ds.num_features, model_params.hidden_dimensions)
