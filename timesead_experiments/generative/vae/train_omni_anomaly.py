...
from timesead.models.generative import OmniAnomaly, OmniAnomalyLoss, OmniAnomalyDetector
...
def get_anomaly_detector(model, val_loader, training, detector_params, _run, save_detector=True):
    ...
    detector = OmniAnomalyDetector(model, **detector_params).to(training.device)
...
def main(model_params, dataset, training, _run, train_detector=True):
    ...
    train_ds, val_ds = load_dataset()
    model = OmniAnomaly(train_ds.num_features, **model_params)

    trainer = train_model(_run, model, train_ds, val_ds)
    early_stop = trainer.hooks['post_validation'][-1]
    model = early_stop.load_best_model(trainer, model, 1)
