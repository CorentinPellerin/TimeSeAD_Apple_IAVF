...
def evaluate_once(train_id, config_updates, detector_param_updates, params, val_ds_params, val_split, seed):
    ...
    model_file     = os.path.join(LOG_DIRECTORY, exp_name_short, str(train_id), 'final_model.pth')
    ...
    # === DEBUG : Afficher les fichiers utilisés pour la validation ===
    print("[DEBUG] Fichiers utilisés pour la validation dans ce split :")
    try:
        if hasattr(val_ds, "test_files_basenames"):
            print(val_ds.test_files_basenames)
        elif hasattr(val_ds, "files"):
            print(val_ds.files)
        elif hasattr(val_ds, "file_names"):
            print(val_ds.file_names)
        else:
            print("Impossible d'extraire la liste des fichiers, ajoute un print dans la classe Dataset si besoin.")
    except Exception as e:
        print(f"Erreur lors de l'affichage des fichiers de validation : {e}")
    # === DEBUG
    ...
    for u, updates in enumerate(detector_param_updates):
        ...
        # SEQUENCE-WISE AD!
        # ##### original_test_seq_lengths = np.asarray(val_ds.sink_transform.parent.dataset.test_lengths)  # might fail for other methods....
        # HACK: Made the test set of the AppleDataset return the ID of the sequence ({0, ..., n}) +1 ({1, ..., n+1}) and negative if normal ({-1, 2, 3, -4, 5, -6, ..., n+1})
        if len(set(labels.numpy().tolist())) > 2 and (labels < 0).sum() > 0:  
            seq_lbl, seq_wltc_id, seq_scores = [], [], []
            for seq_id in set(labels.numpy().tolist()):
                anomalous = int(seq_id >= 1)
                score = scores[labels == seq_id].topk(int((labels == seq_id).sum().div(100).item()))[0].mean()
                seq_lbl.append(anomalous)
                seq_scores.append(score.item())
                seq_wltc_id.append(abs(seq_id))
            labels = torch.tensor(seq_lbl, dtype=labels.dtype)
            scores = torch.tensor(seq_scores, dtype=scores.dtype)
            
            this_logdir = os.path.join(LOG_DIRECTORY, exp_name_short, str(train_id))
            df = pd.DataFrame({'WLTC_ID': seq_wltc_id, 'i.O.?': (labels <= 0).numpy(),'Anomaliebewertung': scores.numpy()})
            df.set_index('WLTC_ID', inplace=True)
            df = df.sort_values(by='WLTC_ID')
            df.to_csv(os.path.join(this_logdir, f'wltc_id_anomaly_score_map_val_{u:02d}.csv'))
        # #################
        ...
        this_logdir = os.path.join(LOG_DIRECTORY, exp_name_short, str(train_id))
        with open(os.path.join(this_logdir, f'val_{u:02d}_{params.validation_metric}_{val_score}'), 'w') as writer:
            writer.write("")
...
def config():
    ...
    params = dict(
        ...
        evaluation_metrics=['best_f1_score', 'auc', 'auprc'],
        batch_size=128,
        device='cpu',
        # Number of processes to train in parallel and CPU threads used for each process
        exp_processes=1, #2
        threads_per_process=1, #6
        test_folds=1,  # was 5
        train_ids=None, #["127", "128", "129", "130", "131", "132", "133", "134", "135", "136", "137", "138", "139", "140", "141", "142", "143", "144", "145", "146", "147", "148", "149", "150", "151", "152", "153", "155", "156"],
        padding=1 #1
    )
    ...
    for val_fold in range(params.test_folds):
        ...
        for test_fold in test_fold_indices:
            ...
            if len(set(labels.numpy().tolist())) > 2 and (labels < 0).sum() > 0:  
                ...
                # this_logdir = os.path.join(LOG_DIRECTORY, exp_name_short, str(train_id))
                # df = pd.DataFrame({'WLTC_ID': labels.abs().numpy(), 'i.O.?': (labels <= 0).numpy(),'Anomaliebewertung': scores.numpy()})
                # df.set_index('WLTC_ID', inplace=True)
                # df.to_csv(os.path.join(this_logdir, 'wltc_id_anomaly_score_map_val.csv'))
            # #################
            for metric in params.evaluation_metrics:
